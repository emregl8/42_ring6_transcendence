(function () {
  ...
  function renderPost(post) {
    ...
    if (post.user) {
      ...
      authorNameEl.textContent = post.user.firstName ? post.user.firstName + ' ' + (post.user.lastName || '') : post.user.username;
      authorAvatarEl.src = post.user.avatar || 'https://ui-avatars.com/api/?name=' + post.user.username + '&background=0D8ABC&color=fff&size=128';

      const timeStr = new Date(post.createdAt).toLocaleTimeString('en-US', {
        hour: '2-digit',
        minute: '2-digit',
      });
      metaEl.innerHTML = dateStr + ' <span class="dot-separator"></span> ' + timeStr;
    }
  ...
  function renderComments(comments) {
    ...
    comments.forEach(function (comment) {
      ...
      if (currentUser && (comment.user.id === currentUser.id || (currentPostId && currentUser.id === comment.postId))) {
